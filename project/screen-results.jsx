...
// Taiwan monthly irradiance baseline (kWh/m²/day), approximate
const TW_IRRADIANCE = {
...
function computeResults(state) {
  ...
  // Goal adjustment multipliers (seasonal weight) — simplified model
  const goalAdj = {
  ...
  const monthlyKwh = irr.map((ghi, i) => Math.round(capacity * ghi * daysInMonth[i] * perfRatio * goalAdj[i]));
  ...
  // FIT rate for small rooftop ~5.7/kWh; grid ~2.5; assume 40% self-consumption
  ...
  const paybackYears = +(outOfPocket / annualRevenue).toFixed(1);

  // Slight degradation over 20y
  const total20yr = Array.from({ length: 20 }, (_, y) => Math.round(annualRevenue * Math.pow(0.995, y)))
    .reduce((a,b) => a+b, 0);

  // Best tilt angle by goal
  const bestAngle = {
    annual: 22, summer: 10, winter: 45, peak: 18, match: 20, roi: 22
  }[state.goal || "summer"];

  const recommendedAngle = state.goal === "summer" ? "朝南，仰角 10°" :
...
function MonthlyChart({ data, highlight }) {
                ...
                <div style={{
                  ...
                  {v === max && (
                    <div className="num" style={{
                      ...
                      fontSize: 11, fontWeight: 700, color: hl ? "#8B5A10" : "var(--green-900)", whiteSpace: "nowrap",
...
function RevenueChart({ annualRevenue, outOfPocket, paybackYears }) {
  const years = 20;
  ...
  const px = y => PAD + (y / years) * (W - PAD * 2);
  const py = net => H - PAD - ((net - minNet) / range) * (H - PAD * 2);
  const zeroY = py(0);
...
function Results({ state, onRestart }) {
  const r = useMemoR(() => computeResults(state), [state]);
  const [tab, setTab] = useStateR("generation");

  return (
    <div data-screen-label="05 Results" style={{ paddingBottom: 40 }}>
      {/* Summary */}
      ...
      {/* Headline numbers */}
      <div className="card elevated" style={{ padding: 36, marginBottom: 28, background:
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 24 }}>
          ...
          <div style={{ borderLeft: "1px solid var(--ink-100)", paddingLeft: 24 }}>
            ...
            <div className="body-sm" style={{ marginTop: 8, fontFamily: "var(--font-num)" }}>
              NT$ {r.total20yr.toLocaleString()}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 20, borderBottom: "1px solid var(--ink-200)" }}>
        {[
          { id: "generation", label: "發電潛力" },
          { id: "investment", label: "投資試算" },
        ].map(t => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 20 }}>
          <div className="card" style={{ padding: 28 }}>
            ...
            <div style={{ marginTop: 20, padding: 14, background: "var(--green-50)", borderRadius: 10, fontSize: 13, color: "var(--ink-700)" }}>
              ★ 根據你選的目標「<b>{state.goal === "summer" ? "夏季發電量最高" : state.goal === "winter" ? "冬季發電量最高" : "全年總發電量最高"}</b>」，夏季（6–8 月）月平均發電量較台灣平均高約 8%。
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div className="card" style={{ padding: 24 }}>
              ...
              <svg viewBox="0 0 160 80" style={{ width: "100%", marginTop: 16 }}>
                ...
                <polygon points={`20,68 ${20 + 80*Math.cos(r.bestAngle*Math.PI/180)},${68 - 80*Math.sin(r.bestAngle*Math.PI/180)} ${40 + 80*Math.cos(r.bestAngle*Math.PI/180)},${68 - 80*Math.sin(r.bestAngle*Math.PI/180)} 40,68`}
                  fill="var(--green-700)" opacity="0.85" />
                <path d={`M 45 68 A 25 25 0 0 0 ${45 + 25*Math.cos((180-r.bestAngle)*Math.PI/180)} ${68 + 25*Math.sin((180-r.bestAngle)*Math.PI/180)}`}
            ...
            <div className="card" style={{ padding: 24 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {[
                  ...
                  { label: "台灣平均（同容量）", value: Math.round(r.annualKwh * 0.92), color: "var(--ink-300)" },
                ].map((row, i) => {
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: 20 }}>
          ...
          <div className="card" style={{ padding: 28 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <BreakdownRow label="安裝費用" value={state.totalCost} color="var(--ink-700)" max={state.totalCost} />
              <BreakdownRow label={`${state.county || "台北市"} 補助`} value={-state.subsidyAmount} color="var(--green-700)" max={state.totalCost} />
          ...
          <div className="card" style={{ padding: 28 }}>
            ...
            <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, paddingTop: 18, borderTop: "1px solid var(--ink-100)" }}>
              <div>
                ...
                <div className="num" style={{ fontSize: 16, fontWeight: 600, color: "var(--danger)", marginTop: 4 }}>−NT$ {Math.round((state.outOfPocket || 400000)/10000)}萬</div>
              </div>
              <div>
                <div className="caption">年均收益</div>
                <div className="num" style={{ fontSize: 16, fontWeight: 600, color: "var(--green-700)", marginTop: 4 }}>+NT$ {Math.round(r.annualRevenue/1000)}K</div>
              </div>
              <div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: "radial-gradient(circle, rgba(232,165,60,0.2), transparent 70%)",
        }} />
        ...
        <div style={{ display: "flex", gap: 10, position: "relative" }}>
          ...
          <button className="btn" style={{ background: "var(--amber)", color: "var(--green-900)", boxShadow: "0 4px 0 #8B5A10" }}>
            下載評估報告
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M8 2 L8 11 M4 7 L8 11 L12 7 M3 14 L13 14"/></svg>
          </button>
        </div>
      </div>
...
function BreakdownRow({ label, value, color, max }) {
