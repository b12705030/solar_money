...
export function StatBlock({
  label, value, unit, tip, size = 'md', accent = 'green',
}: {
  label: string; value: string | number; unit?: string;
  tip?: string; size?: 'sm' | 'md' | 'lg'; accent?: 'green' | 'amber';
}) {
  const sizes = { sm: { num: 28, lbl: 12 }, md: { num: 44, lbl: 13 }, lg: { num: 64, lbl: 14 } }[size];
...
export function SunIcon({ size = 20, color = 'currentColor' }: { size?: number; color?: string }) {
  return (
...
export function ChevronIcon({ dir = 'right', size = 16 }: { dir?: 'right' | 'left' | 'up' | 'down'; size?: number }) {
  const rot = { right: 0, left: 180, up: -90, down: 90 }[dir];
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.8" style={{ transform: `rotate(${rot}deg)`, flexShrink: 0 }}>
